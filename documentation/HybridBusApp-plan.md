...
- KPI tab as a nested decision workspace with three subtabs: `Executive Decision`, `Engineering Scorecard`, and `Robustness`. The Executive view gives the shortest qualified architecture recommendation; the Scorecard exposes first-principles feasibility gates and like-for-like metrics; the Robustness view separates nominal evidence from operational scenarios that have not been simulated.
