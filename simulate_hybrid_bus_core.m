...
componentMass = Input.Battery1.Mass_kg + Input.Battery2.Mass_kg + ...
    2*Input.Motor.Mass_kg + Input.Genset.Mass_kg + 2*Input.FinalDrive.Mass_kg;
...
regenLimit = min(2*perMotorMechLimit*Input.FinalDrive.RegenEfficiency, ...
    min(Input.Battery1.MaxRegen_kW,Input.Battery2.MaxRegen_kW));
PWheelDelivered = min(max(PWheelDemand,-regenLimit),tractionLimit);
unmetTraction = max(0,PWheelDemand-PWheelDelivered);
unmetRegen = max(0,-PWheelDemand+min(PWheelDelivered,0));
PMotorDC = zeros(n,1);
...
Signals.Wheel=struct('Demand_kW',PWheelDemand,'Delivered_kW',PWheelDelivered, ...
    'UnmetTraction_kW',unmetTraction,'UnmetRegen_kW',unmetRegen);
...
Signals.Controller=struct('ActiveBattery',active,'StandbyBattery',3-active,'Mode',mode, ...
    'BatteryRoleSwitchSOE',switchSOE*ones(n,1));
...
gridEnergy=(max(0,E1(1)-E1(end))+max(0,E2(1)-E2(end)))/Input.Vehicle.GridChargeEfficiency;
...
usableInitial=max(0,E1(1)-Input.Battery1.MinSOE*Input.Battery1.UsableEnergy_kWh)+ ...
    max(0,E2(1)-Input.Battery2.MinSOE*Input.Battery2.UsableEnergy_kWh);
...
Results=struct('Metadata',struct('DatabaseFilename',Input.DatabaseFile, ...
    ...
    'ModelVersion','1.0.0','SimulationTimestamp',datetime('now')), ...
